...
namespace GoogleCloudStreamingSpeechToText {
    ...
    public class CustomStreamingRecognizer : MonoBehaviour {
        ...
        private int _previousResultEndTime = 0;
        ...
        private const int StreamingLimit = 5000; // 300000 almost 5 minutes
        ...
        public void Initialize() {
           ...
           string credentialsPath =  Path.Combine(Application.persistentDataPath, CredentialFileName);
           ...
           //soluation is to manually copy the file to -> adb push <local file> <remote location>
            ...
            Debug.Log(Environment.GetEnvironmentVariable("GOOGLE_APPLICATION_CREDENTIALS"));

            if (!File.Exists(credentialsPath))
            {
                Debug.LogError(credentialsPath);
                ...
                Debug.Log("credential file exist " + credentialsPath);
            }
        ...
        private IEnumerator RequestMicrophoneAuthorizationAndStartListening() {
            ...
            Debug.Log("[Google] RequestMicrophoneAuthorizationAndStartListening ");

            while (!Application.HasUserAuthorization(UserAuthorization.Microphone)) {
                yield return Application.RequestUserAuthorization(UserAuthorization.Microphone);
            }

            Debug.Log("[Google] RequestMicrophoneAuthorizationAndStartListening Initialize");

            InitializeMicrophoneAndBeginStream();

            Debug.Log("[Google] RequestMicrophoneAuthorizationAndStartListening Done");
        }
        ...
        private async Task HandleTranscriptionResponses() {
            ...
            //if(t!=null) t.text = "New :" + fire_start_time + "s";
            ...
            while (await _streamingCall.ResponseStream.MoveNext(default)) {
                ...
                if (result.IsFinal) {
                    ...
                    var time = Time.time - fire_start_time;
                    if (t != null) t.text =transcript;
                    //if (t!=null) t.text = "Final :" +  transcript + "\n time from last final result" + time.ToString() + "s";
                    ...
                    if (returnInterimResults) {
                        ...
                        var time = Time.time - fire_start_time;
                        if (t != null) t.text = transcript;
                        //if (t!=null) t.text = "Interim :" + transcript + "\n" + time.ToString() + "s";
                    }
                }
            }
        }
        ...
        private async void StreamingMicRecognizeAsync() {
            ...
            //for some reason this is causing a crash on android
